...
class USPS(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, download=False):
        ...
        # print("Before transform, USPS raw min/max:", self.train_data.min(), self.train_data.max())
        if self.train:
    ...
    def __getitem__(self, index):
        ...
        # print("Before transform, USPS tensor min/max:", 
        #     img.min().item(), img.max().item())
        if self.transform is not None:
            img = self.transform(img)
            # if index == 0:  # 只打印一次
            #     print("After transform, USPS tensor min/max:", 
            #         img.min().item(), img.max().item())
